...
string trainSVM(TrainData* trainingData, double c, double gamma, double degree = 0, double coef0 = 0, double nu = 0, double p = 0);
void getTestFiles(string path, vector<string>& files);
int testSVM(int& mistakes, int* result, double c, double gamma);
...
int main()
{
	...
	Ptr<TrainData> trainingData = TrainData::create(trainingImages, ROW_SAMPLE, trainingLabels);
	...
	for (int i = 0; i < sizeof(cValues); i++) {
		for (int j = 0; j < sizeof(gammaValues); j++) {
			...
			int num_tested = testSVM(mistakes, result, cValues[i], gammaValues[j]);
...
string trainSVM(TrainData* trainingData, double c, double gamma, double degree, double coef0, double nu, double p) {
	string filename = "gridsearch";
	filename.append("c").append(to_string(c)).append("g").append(to_string(gamma)).append(".xml");
...
int testSVM(int& mistakes, int* result, double c, double gamma) {
	...
	string modelpath = "gridsearch";
	modelpath.append("c").append(to_string(c)).append("g").append(to_string(gamma)).append(".xml");
